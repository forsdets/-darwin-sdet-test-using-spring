@darwin_api_tests @get @negative_path
Feature: Validate the JSON Placeholder API's GET request - Negative Scenarios

  Background:
    Given baseUri is available

  @get_request_with_missing_mandatory_field
  Scenario: Validate the GET Request by updating an existing user data with missing mandatory fields
    Given user sets Accept header to application/json
    And user sets body to {"name": "testName"}
    When user makes GET request on the endpoint /users
    Then response code should be 400
#   Currently the test API is not giving any negative response for missing mandatory fields

  @get_request_with_missing_mandatory_value
  Scenario: Validate the GET Request by updating an existing user data with missing mandatory value
    Given user sets Accept header to application/json
...
#   Currently the test API is not giving any negative response for missing mandatory fields
  ...
  @get_request_with_invalid_mandatory_header_values
  Scenario Outline: Validate the GET Request by updating an existing user data with invalid header fields
